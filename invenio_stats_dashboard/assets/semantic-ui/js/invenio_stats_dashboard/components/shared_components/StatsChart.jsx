	...
	formatter: function (params) {
		...
		sortedParams.forEach((param) => {
			...
			result +=
				...
				displayName +
				": " +
				formatNumber(value, valueType === "filesize" ? "filesize" : "compact", {
					...
					areaStyle:
						(chartType || series.type || "bar") === "line"
							? {
									...
									color: CHART_COLORS.secondary[colorIndex][1],
...
const StatsChart = ({
	...
	const getEffectiveDefaultLineDisplay = useMemo(() => {
		...
		if (
			displaySeparately &&
			display_subcounts &&
			display_subcounts[displaySeparately]
		) {
	...
	const [isStackedLine, setIsStackedLine] = useState(
	...
	return (
		<Container fluid className="stats-chart-layout">
			{title && (
				<Header
					as="h3"
					attached="top"
					fluid
					textAlign="center"
					className="rel-mt-1 stats-chart-header"
				>
					<FilterSelector
						data={data}
			...
			<Segment
				className={`stats-chart ${classnames} rel-mb-1 rel-mt-0 stats-chart-segment`}
				attached="bottom"
				fluid
				role="region"
			...
			>
				{showControls && !isLoading && (
					...
					>
						{showSeriesControls && (
							<>
								<Button.Group className="stats-chart-series-controls separated computer large monitor tablet only">
									{seriesSelectorOptions &&
								...
								<Dropdown
									id="stats-chart-metric-selector"
									className="stats-chart-metric-selector mobile only centered"
