...
const TopUsersMultiDisplay = ({
  title = undefined,
  icon = "user",
  pageSize = 10,
  headers = [i18next.t("User"), i18next.t("Visits")],
  default_view,
  ...
  const transformedData = stats.users?.slice(0, pageSize).map((user, index) => ({
    name: user.name,
    value: user.count,
  ...
  const remainingItems = stats.users?.slice(pageSize) || [];
  const otherData = remainingItems.length > 0 ? remainingItems.reduce((acc, user) => {
    acc.value += user.count;
    acc.percentage += user.percentage;
    return acc;
  }, {
    id: "other",
    name: "Other",
...
TopUsersMultiDisplay.propTypes = {
