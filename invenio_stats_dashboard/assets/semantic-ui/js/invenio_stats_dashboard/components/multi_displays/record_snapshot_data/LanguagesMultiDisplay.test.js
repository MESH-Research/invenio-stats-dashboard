...
describe('LanguagesMultiDisplay', () => {
  ...
  describe('transformMultiDisplayData (helper function)', () => {
    it('should return empty data when input is null', () => {
      const result = transformMultiDisplayData(null, null, 10, 'metadata.language.id');
    ...
    it('should return empty data when input is undefined', () => {
      const result = transformMultiDisplayData(undefined, null, 10, 'metadata.language.id');
    ...
    it('should return empty data when input is not an array', () => {
      const result = transformMultiDisplayData('not an array', null, 10, 'metadata.language.id');
    ...
    it('should transform valid languages data correctly', () => {
      const mockData = [
        {
          id: 'english',
          name: 'English',
      ...
      const result = transformMultiDisplayData(mockData, null, 2, 'metadata.language.id');
    ...
    it('should handle data with missing or invalid values', () => {
      ...
      const result = transformMultiDisplayData(mockData, null, 10, 'metadata.language.id');
    ...
    it('should create correct links for each language', () => {
      const mockData = [
        {
          id: 'english',
          name: 'English',
      ...
      expect(result.transformedData[0].link).toBe('/search?q=metadata.language.id:"english"');
    });
    ...
    it('should create transformed data with all expected properties', () => {
      ...
      expect(firstItem).toHaveProperty('name', 'English');
      expect(firstItem).toHaveProperty('value', 100);
      expect(firstItem).toHaveProperty('percentage', 67); // 100/150 * 100 rounded
      expect(firstItem).toHaveProperty('id', 'english');
      expect(firstItem).toHaveProperty('link', '/search?q=metadata.language.id:"english"');
      ...
      expect(secondItem).toHaveProperty('name', 'Spanish');
      expect(secondItem).toHaveProperty('value', 50);
      expect(secondItem).toHaveProperty('percentage', 33); // 50/150 * 100 rounded
      expect(secondItem).toHaveProperty('id', 'spanish');
      expect(secondItem).toHaveProperty('link', '/search?q=metadata.language.id:"spanish"');
  ...
  describe('assembleMultiDisplayRows (helper function)', () => {
    it('should assemble rows correctly with transformed data only', () => {
      const transformedData = [
        {
          ...
          value: 100,
          percentage: 80,
          link: '/search?q=metadata.language.id:english'
        },
        {
          name: 'Spanish',
          value: 25,
          percentage: 20,
          link: '/search?q=metadata.language.id:spanish'
        }
      ];
      ...
      expect(rows[0][1].props.href).toBe('/search?q=metadata.language.id:english');
      expect(rows[0][1].props.children).toBe('English');
      expect(rows[0][2]).toBe('100 (80%)');

      expect(rows[1][0]).toBe(null);
      expect(rows[1][1].type).toBe('a');
      expect(rows[1][1].props.href).toBe('/search?q=metadata.language.id:spanish');
      expect(rows[1][1].props.children).toBe('Spanish');
  ...
  describe('filterSeriesArrayByDate integration', () => {
    it('should filter languages data by date range correctly', () => {
      ...
      const filteredData = filterSeriesArrayByDate(mockLanguagesData, dateRange, true);
    ...
    it('should work with the actual component data flow', () => {
      ...
      const result = transformMultiDisplayData(filteredData, null, 10, 'metadata.language.id');
  ...
  describe('Component rendering with different display types', () => {
    ...
    it('should render proper table structure with headers', async () => {
      ...
      expect(headerCells[1]).toHaveTextContent('Language');
    ...
    it('should render table rows with proper structure', async () => {
      ...
      expect(dataRows).toHaveLength(3); // 3 languages
      ...
      // Check that the first data cell contains the language name
      expect(firstRowCells[1]).toHaveTextContent('English');
    ...
    it('should render with custom headers properly', async () => {
      ...
      expect(headerCells[1]).toHaveTextContent('Custom Language');
