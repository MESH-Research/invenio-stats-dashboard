...
const TopOperatingSystemsMultiDisplay = ({
  title = undefined,
  icon = "desktop",
  pageSize = 10,
  headers = [i18next.t("Operating System"), i18next.t("Visits")],
  default_view,
  ...
  const transformedData = stats.operatingSystems?.slice(0, pageSize).map((os, index) => ({
    name: os.name,
    value: os.count,
    percentage: os.percentage,
    id: os.name.toLowerCase().replace(/\s+/g, '-'),
    itemStyle: {
      color: CHART_COLORS.secondary[index % CHART_COLORS.secondary.length]
    }
  })) || [];

  const remainingItems = stats.operatingSystems?.slice(pageSize) || [];
  const otherData = remainingItems.length > 0 ? remainingItems.reduce((acc, os) => {
    acc.value += os.count;
    acc.percentage += os.percentage;
    return acc;
  }, {
    id: "other",
    name: "Other",
...
TopOperatingSystemsMultiDisplay.propTypes = {
...
export { TopOperatingSystemsMultiDisplay };