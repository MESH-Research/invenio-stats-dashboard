...
const StatsDashboardLayout = ({
  ...
  const showTitle = ["true", "True", "TRUE", "1", true].includes(dashboardConfig?.show_title);
  ...
  const [granularity, setGranularity] = useState(dashboardConfig?.default_granularity || "day");
  ...
  useEffect(() => {
    setPageDateRangePhrase(
      dashboardConfig.layout?.tabs?.find((tab) => tab.name === selectedTab)?.date_range_phrase
    );
    ...
    <StatsDashboardProvider value={contextValue}>
      {showSubheader && (
        <div className="ui container fluid page-subheader-outer compact stats-dashboard-header ml-0-mobile mr-0-mobile">
          <div className="ui container stats-dashboard page-subheader flex align-items-center justify-space-between">
            <h1 className="ui huge header">{dashboardConfig?.title || i18next.t("Statistics")}</h1>
            {showDescription && (
      ...
      >
        ...
        <Grid.Row>
          <Grid.Column
            width={3}
            className={`${sidebarClassNames} stats-dashboard-sidebar rel-mt-0 computer widescreen large screen only`}
          >
            {showTitle && (
              <h2 className="stats-dashboard-header computer widescreen large screen only">
                {dashboardConfig.title || i18next.t("Statistics")}
              </h2>
            )}
            <Menu
              fluid
              vertical
              className="stats-dashboard-sidebar-menu rel-mt-2 rel-mb-2 theme-primary-menu computer widescreen large screen only"
            >
          ...
          >
            <div id="mobile-action-menu-toggle">
              <Button icon onClick={() => setActionMenuOpen(!actionMenuOpen)}>
                <Icon name={actionMenuOpen ? "chevron down" : "chevron up"} className="fitted" />
              </Button>
            </div>
            <Grid>
              ...
              <Grid.Row className="mb-0 mt-0 pb-0">
                <Grid.Column className="flex">
                  <Dropdown
                    ...
                    text={
                      <>
                        <Icon name={availableTabs.find((tab) => tab.name === selectedTab)?.icon} />
                        <b>{availableTabs.find((tab) => tab.name === selectedTab)?.label}</b>
                      </>
                    }
                  />
                </Grid.Column>
