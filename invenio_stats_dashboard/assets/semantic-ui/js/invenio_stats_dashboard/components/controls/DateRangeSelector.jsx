...
const validateDate = (year, month, day, fieldPrefix) => {
	...
	if (!year || !/^\d{4}$/.test(year)) {
		...
		if (yearNum < 1900 || yearNum > 2100) {
			errors[`${fieldPrefix}Year`] = i18next.t(
	...
	if (!month || !/^\d{2}$/.test(month)) {
		...
		if (monthNum < 1 || monthNum > 12) {
			errors[`${fieldPrefix}Month`] = i18next.t(
	...
	if (
		!errors[`${fieldPrefix}Year`] &&
		!errors[`${fieldPrefix}Month`] &&
		!errors[`${fieldPrefix}Day`]
	) {
		try {
			const yearNum = parseInt(year, 10);
			const monthNum = parseInt(month, 10);
			...
			if (
				testDate.getUTCFullYear() !== yearNum ||
									...
									<Grid.Column width={4} className="pr-5 pl-5">
										<Form.Field error={!!errors.startMonth}>
											...
											<Input
												...
												onBlur={(e) =>
													setStartMonth(formatMonth(e.target.value))
												}
												maxLength={2}
											/>
											{errors.startMonth && (
...
const DateRangeSelector = ({
	...
	useEffect(() => {
		...
		if (dateRange?.start && dateRange?.end) {
			const expandedDateRange = expandDateRangeToGranularity(
				dateRange,
				granularity,
			);
	...
	const getDisplayText = () => {
		...
		const selectedOption = currentPeriodOptions.find(
			(option) => option.value === currentSelectedOption,
		);
