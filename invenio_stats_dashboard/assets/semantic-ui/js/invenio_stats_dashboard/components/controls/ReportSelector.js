...
import { packageStatsAsCompressedJson } from "../../api/jsonDownloadSerializer";
		...
		try {
			...
			if (selectedReport === "json") {
				...
				if (!stats || stats.length === 0) {
					throw new Error(
						"No statistics data available for download. Please ensure the dashboard has loaded data.",
					);
				}
				...
				console.log(
					"Successfully downloaded JSON report from cached data as tar.gz archive",
				);
				return;
			}
	...
	return (
		<Segment className="stats-dashboard-report-selector rel-mt-1 rel-mb-1 communities-detail-stats-sidebar-segment">
			...
			>
				...
				<Popup
					content={i18next.t(
						"Includes complete time-series data for the currently selected time period.",
					)}
					trigger={
						<Icon
							name="info circle"
