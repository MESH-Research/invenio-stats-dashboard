...
class DataSeriesMemoryEstimator:
    ...
    def __init__(
        ...
        self.k_page_overhead = float(cfg.get("STATS_DATA_SERIES_PAGE_OVERHEAD", 0.85))
        self.k_series_overhead = float(
            cfg.get("STATS_DATA_SERIES_SERIES_OVERHEAD", 1.5)
        )
        self.safety_factor = float(cfg.get("STATS_DATA_SERIES_MEM_SAFETY_FACTOR", 1.2))
        ...
        self.avg_doc_bytes = int(cfg.get("STATS_DATA_SERIES_AVG_DOC_BYTES", 2048))
    ...
    def re_estimate_after_first_page(self, days_processed: int) -> None:
        ...
        if self.initial_rss <= 0 or self.rss_with_page is None or days_processed <= 0:
            return
    ...
    def update_page_size(self, days_processed: int) -> int:
        ...
        if new_page_size < old_page_size and self.budget_bytes > 0:
            try:
                ...
                rss = 0
                current_app.logger.error(f"Error getting RSS memory usage: {e}")
    ...
    def adjust_page_size(self, current_used_bytes: int | None = None) -> int:
        ...
        # excludes the current page, which is already processed.
        if self.total_count is not None:
            ...
            remaining_series_growth = self.series_bytes_per_day * (
                self.days_processed + self.current_page_size
            )

        additional_memory_needed = self.per_page_bytes + remaining_series_growth
        ...
        if additional_memory_needed <= 0:
            ...
            return max(
                self.page_size_min, min(self.current_page_size, self.page_size_max)
            )
        ...
        linear_cap = max(
            self.page_size_min,
            int(
                self.current_page_size * (free_bytes / float(additional_memory_needed))
            ),
        )
        ...
        adjusted = max(
            self.page_size_min, min(sqrt_proposed, self.page_size_max, linear_cap)
        )
...
class DataSeriesQueryBase(Query):
    ...
    def run(
        ...
        try:
            ...
            count_search = Search(using=self.client, index=final_search_index).query(
            ...
            if category == "global":
                ...
                series_keys = [category]
            series_set = self.transformer_class(documents=[], series_keys=series_keys)
