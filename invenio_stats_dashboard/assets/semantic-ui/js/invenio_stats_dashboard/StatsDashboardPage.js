...
const StatsDashboardPage = ({
  ...
  const renderComponent = (componentConfig) => {
    ...
    const mobileWidth = componentConfig.props?.mobile ?? componentConfig.mobile ?? 16;
    const tabletWidth =
      componentConfig.props?.tablet ?? componentConfig.tablet ?? componentConfig.width;
  ...
  const noDataMessage =
    !isLoading && !isUpdating && !error && (!stats || !!statsAreEmpty || !!first_run_incomplete) ? (
      ...
      <Grid.Row className="rel-mt-0 rel-mb-0 pt-0 pb-0">
        <Grid.Column width={16} className="centered">
          <span className="stats-agg-in-progress-message">
            {i18next.t("Update calculation in progress. Check back soon for updated data.")}
          </span>
        </Grid.Column>
      </Grid.Row>
    ) : null;
  ...
  return (
    <Grid
      ...
      {...otherProps}
    >
      <Grid.Row className="pb-0 large screen computer only">
        <Grid.Column width={16} className="centered">
      ...
      {currentTab.rows.map((row, index) => (
        <Grid.Row key={row.name} className={`stats-dashboard-row pb-0 pt-0 ${row.name}`}>
