...
import { fetchStats, updateState, updateStatsFromCache } from './api/api';
...
describe('StatsDashboardLayout with caching', () => {
  ...
  it('should display data when loaded', async () => {
    ...
    fetchStats.mockImplementation(({ onStateChange, isMounted }) => {
      // Simulate cached stale data callback
      if (onStateChange && isMounted && isMounted()) {
        onStateChange({
          type: 'stale_and_updating',
          stats: mockTransformedStats,
          isLoading: false,
          isUpdating: true,
          error: null
...
describe('StatsDashboardLayout - Cache update event handling', () => {
  beforeEach(() => {
    const { updateState, updateStatsFromCache } = require('./api/api');

    updateState.mockImplementation((onStateChange, isMounted, stateName, stats, additionalProps) => {
      if (onStateChange && (!isMounted || isMounted())) {
        onStateChange({
          stats,
          isLoading: false,
    ...
    updateStatsFromCache.mockImplementation((currentStats, updatedData, year) => {
  ...
  it('should update stats when statsCacheUpdated event is received', async () => {
    ...
    // Wait for React to re-render and re-run the useEffect that recreates the event listener
    // The useEffect depends on [stats, currentYearLastUpdated], so it should re-run when stats changes
    // We need to give React time to process the state update and re-run effects
    await new Promise(resolve => setTimeout(resolve, 100));

    // At this point, the useEffect should have re-run and recreated the event listener with populated stats
    // Clear the mock call count before dispatching the event
    // so we can tell that it's called by the event, not by initial load
    updateStatsFromCache.mockClear();
    updateState.mockClear();
