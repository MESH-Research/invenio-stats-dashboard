...
const TopLanguagesMultiDisplay = ({
  ...
  icon = "language",
  pageSize = 10,
  headers = [i18next.t("Language"), i18next.t("Visits")],
  default_view,
  ...
  const transformedData = stats.languages?.slice(0, pageSize).map((language, index) => ({
    name: language.name,
  ...
  const remainingItems = stats.languages?.slice(pageSize) || [];
  const otherData = remainingItems.length > 0 ? remainingItems.reduce((acc, language) => {
    acc.value += language.count;
    acc.percentage += language.percentage;
    return acc;
  }, {
    id: "other",
    name: "Other",
