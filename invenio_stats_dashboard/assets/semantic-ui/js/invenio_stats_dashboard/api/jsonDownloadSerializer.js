 ...
 */
const serializeStatsToJson = (stats, dateBasis) => {
	...
	stats.forEach((yearBlock) => {
		...
		const { year, ...yearData } = yearBlock;

		// Merge each category
		Object.keys(response).forEach((category) => {
			const categoryKey = convertCategoryKey(category, dateBasis);
			if (yearData[categoryKey]) {
				mergeCategoryData(response[category], yearData[categoryKey]);
			}
		});
	});
 ...
 */
const createTarGzJsonBlob = (stats, dateBasis, jsonFilename) => {
	// Serialize the stats data to JSON format
	const jsonData = serializeStatsToJson(stats, dateBasis);
 ...
 */
const packageStatsAsCompressedJson = async (
	stats,
	communityId,
	dashboardType,
	dateBasis,
	...
	try {
		...
		const blob = createTarGzJsonBlob(stats, dateBasis, jsonFilename);
...
export {
	createTarGzJsonBlob,
	packageStatsAsCompressedJson,
	serializeStatsToJson,
};
