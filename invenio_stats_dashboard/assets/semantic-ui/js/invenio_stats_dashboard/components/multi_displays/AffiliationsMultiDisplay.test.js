...
import { AffiliationsMultiDisplay } from './AffiliationsMultiDisplay';
...
describe('AffiliationsMultiDisplay', () => {
  ...
  describe('transformMultiDisplayData (helper function)', () => {
    ...
    it('should transform valid affiliations data correctly', () => {
      const mockData = [
        {
          id: 'harvard-university',
          name: 'Harvard University',
    ...
    it('should create correct links for each affiliation', () => {
      const mockData = [
        {
          id: 'harvard-university',
          name: 'Harvard University',
    ...
    it('should create transformed data with all expected properties', () => {
      ...
      expect(firstItem).toHaveProperty('name', 'Harvard University');
      ...
      expect(secondItem).toHaveProperty('name', 'MIT');
  ...
  describe('assembleMultiDisplayRows (helper function)', () => {
    it('should assemble rows correctly with transformed data only', () => {
      const transformedData = [
        {
          ...
          value: 100,
          percentage: 80,
          link: '/search?q=metadata.affiliations.affiliation:harvard-university'
        },
        {
          name: 'MIT',
          value: 25,
          percentage: 20,
          link: '/search?q=metadata.affiliations.affiliation:mit'
        }
      ];
      ...
      expect(rows[0][1].props.href).toBe('/search?q=metadata.affiliations.affiliation:harvard-university');
      expect(rows[0][1].props.children).toBe('Harvard University');
      expect(rows[0][2]).toBe('100 (80%)');

      expect(rows[1][0]).toBe(null);
      expect(rows[1][1].type).toBe('a');
      expect(rows[1][1].props.href).toBe('/search?q=metadata.affiliations.affiliation:mit');
      expect(rows[1][1].props.children).toBe('MIT');
    ...
    it('should handle data without links', () => {
      ...
      expect(rows[0][0]).toBe(null);
      expect(rows[0][1]).toBe('Harvard University');
  ...
  describe('filterSeriesArrayByDate integration', () => {
    it('should filter affiliations data by date range correctly', () => {
      const mockAffiliationsData = [
        {
          id: 'harvard-university',
          name: 'Harvard University',
          data: [
  ...
  describe('Component rendering with different display types', () => {
    ...
    it('should render with custom title and icon', () => {
      render(<AffiliationsMultiDisplay title="Custom Title" icon="university" default_view="list" />);
    ...
    it('should render proper table structure with headers', () => {
      ...
      expect(headerCells[1]).toHaveTextContent('Affiliation');
    ...
    it('should render table rows with proper structure', () => {
      ...
      expect(dataRows).toHaveLength(3); // 3 affiliations
      ...
      // Check that the first data cell contains the affiliation name
      expect(firstRowCells[1]).toHaveTextContent('Harvard University');
    ...
    it('should render with custom headers properly', () => {
      ...
      expect(headerCells[1]).toHaveTextContent('Custom Affiliation');
