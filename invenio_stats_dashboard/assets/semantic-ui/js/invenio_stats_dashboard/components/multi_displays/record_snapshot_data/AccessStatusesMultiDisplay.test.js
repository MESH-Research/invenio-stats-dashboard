...
describe('AccessStatusesMultiDisplay', () => {
  ...
  describe('transformMultiDisplayData (helper function)', () => {
    it('should return empty data when input is null', () => {
      const result = transformMultiDisplayData(null, null, 10, 'metadata.access_status.id');
    ...
    it('should return empty data when input is undefined', () => {
      const result = transformMultiDisplayData(undefined, null, 10, 'metadata.access_status.id');
    ...
    it('should return empty data when input is not an array', () => {
      const result = transformMultiDisplayData('not an array', null, 10, 'metadata.access_status.id');
    ...
    it('should transform valid access rights data correctly', () => {
      const mockData = [
        {
          id: 'open',
          name: 'Open Access',
      ...
      const result = transformMultiDisplayData(mockData, null, 2, 'metadata.access_status.id');
    ...
    it('should handle data with missing or invalid values', () => {
      ...
      const result = transformMultiDisplayData(mockData, null, 10, 'metadata.access_status.id');
    ...
    it('should create correct links for each access right', () => {
      const mockData = [
        {
          id: 'open',
          name: 'Open Access',
    ...
    it('should create transformed data with all expected properties', () => {
      ...
      expect(firstItem).toHaveProperty('name', 'Open Access');
      ...
      expect(secondItem).toHaveProperty('name', 'Restricted Access');
  ...
  describe('assembleMultiDisplayRows (helper function)', () => {
    ...
    it('should handle data without links', () => {
      ...
      expect(rows[0][0]).toBe(null);
      expect(rows[0][1]).toBe('Open Access');
  ...
  describe('filterSeriesArrayByDate integration', () => {
    it('should filter access rights data by date range correctly', () => {
      ...
      // Open Access should have the latest data point within range (Jan 15)
      expect(filteredData[0].id).toBe('open');
      expect(filteredData[0].data).toHaveLength(1);
      expect(filteredData[0].data[0][1]).toBe(150);

      // Restricted Access should have the latest data point within range (Jan 20)
      expect(filteredData[1].id).toBe('restricted');
    ...
    it('should work with the actual component data flow', () => {
      ...
      const result = transformMultiDisplayData(filteredData, null, 10, 'metadata.access_status.id');
  ...
  describe('Component rendering with different display types', () => {
    ...
    it('should render proper table structure with headers', async () => {
      ...
      expect(headerCells[1]).toHaveTextContent('Access Right');
    ...
    it('should render table rows with proper structure', async () => {
      ...
      expect(dataRows).toHaveLength(3); // 3 access rights
      ...
      // Check that the first data cell contains the access right name
      expect(firstRowCells[1]).toHaveTextContent('Open Access');
    ...
    it('should render with custom headers properly', async () => {
      ...
      expect(headerCells[1]).toHaveTextContent('Custom Access Rights');
