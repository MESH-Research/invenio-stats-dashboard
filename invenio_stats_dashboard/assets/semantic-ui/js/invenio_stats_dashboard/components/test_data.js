...
const generateMetricDataPoints = (
	baseValue,
	variance,
	startValue = 0,
	dates,
...
const generateCumulativeDataPoints = (
	startValue,
	dailyBaseValue,
	dailyVariance,
	dates,
...
const generateDataVolumeDataPoints = (
	baseValue,
	variance,
	startValue = 0,
	dates,
...
const generateCumulativeDataVolumeDataPoints = (
	startValue,
	dailyBaseValue,
	dailyVariance,
	dates,
...
const createRecordMetrics = (isSnapshot = false, dates, year) => {
	const dataType = isSnapshot
		? generateCumulativeDataPoints
		: generateMetricDataPoints;
	const dataVolumeType = isSnapshot
		? generateCumulativeDataVolumeDataPoints
		: generateDataVolumeDataPoints;
...
const createUsageMetrics = (isSnapshot = false, dates, year) => {
	...
	return {
		...
		downloadUniqueParents: [
			createGlobalSeries(
				dataType(60000, 30, 15, dates, year),
				"bar",
				"number",
				year,
			),
		...
		downloadUniqueRecords: [
			createGlobalSeries(
				dataType(70000, 35, 18, dates, year),
				"bar",
				"number",
				year,
			),
...
const createSubcountSeries = (
			...
			if (globalValue <= 0) {
				...
				for (let i = 0; i < itemCount; i++) {
					if (i === itemCount - 1) {
						...
						const minValue = 1;
						const maxPortion = Math.max(
							minValue,
							Math.floor(
								(remainingValue - (itemCount - i - 1)) / (itemCount - i),
							),
						);
	...
	const recordsDistribution = generateBreakdownData(
		globalRecords,
		items.length,
		dates,
	);
	const parentsDistribution = generateBreakdownData(
		globalParents,
		items.length,
		dates,
	);
	const fileCountDistribution = generateBreakdownData(
		globalFileCount,
		items.length,
		dates,
	);
	const dataVolumeDistribution = generateBreakdownData(
		globalDataVolume,
		items.length,
		dates,
	);
	...
	return {
		...
		fileCount: items.map((item, itemIndex) => {
			const dataPoints = fileCountDistribution.map(
		...
		dataVolume: items.map((item, itemIndex) => {
			const dataPoints = dataVolumeDistribution.map(
				(distribution, dateIndex) => {
					const date = dates[dateIndex];
					const value = distribution[itemIndex] || 0;
...
const createUsageSubcountSeries = (
	categoryName,
	items,
	isSnapshot = false,
	globalMetrics = null,
	...
	const globalViewUniqueRecords =
		globalMetrics.viewUniqueRecords?.[0]?.data || [];
	const globalDownloadUniqueFiles =
		globalMetrics.downloadUniqueFiles?.[0]?.data || [];
	const globalDataVolume = globalMetrics.dataVolume[0]?.data || [];
	const globalViewVisitors = globalMetrics.viewVisitors?.[0]?.data || [];
	const globalDownloadVisitors =
		globalMetrics.downloadVisitors?.[0]?.data || [];

	// Generate breakdown distributions
	const viewUniqueRecordsDistribution = generateBreakdownData(
		globalViewUniqueRecords,
		items.length,
		dates,
	);
	const downloadUniqueFilesDistribution = generateBreakdownData(
		globalDownloadUniqueFiles,
		items.length,
		dates,
	);
	const dataVolumeDistribution = generateBreakdownData(
	...
	const viewVisitorsDistribution = generateBreakdownData(
		globalViewVisitors,
		items.length,
		dates,
	);
	const downloadVisitorsDistribution = generateBreakdownData(
		globalDownloadVisitors,
		items.length,
		dates,
	);
	...
	return {
		...
		viewUniqueRecords: items.map((item, itemIndex) => {
			const dataPoints = viewUniqueRecordsDistribution.map(
				(distribution, dateIndex) => {
					const date = dates[dateIndex];
					const value = distribution[itemIndex] || 0;
					return createDataPoint(date, value, "number", year);
		...
		downloadUniqueFiles: items.map((item, itemIndex) => {
			const dataPoints = downloadUniqueFilesDistribution.map(
				(distribution, dateIndex) => {
					const date = dates[dateIndex];
					const value = distribution[itemIndex] || 0;
					return createDataPoint(date, value, "number", year);
		...
		viewVisitors: items.map((item, itemIndex) => {
			const dataPoints = viewVisitorsDistribution.map(
				(distribution, dateIndex) => {
					const date = dates[dateIndex];
					const value = distribution[itemIndex] || 0;
					return createDataPoint(date, value, "number", year);
		...
		downloadVisitors: items.map((item, itemIndex) => {
			const dataPoints = downloadVisitorsDistribution.map(
				(distribution, dateIndex) => {
					const date = dates[dateIndex];
					const value = distribution[itemIndex] || 0;
					return createDataPoint(date, value, "number", year);
	...
	return {
		...
		recordDeltaDataCreated: (() => {
			...
			return {
				...
				resourceTypes: createSubcountSeries(
					"resourceTypes",
					resourceTypesItems,
					false,
					globalMetrics,
				...
				accessStatuses: createSubcountSeries(
					"accessStatuses",
					accessStatusesItems,
					false,
					globalMetrics,
				...
				languages: createSubcountSeries(
					"languages",
					languagesItems,
					false,
					globalMetrics,
				...
				affiliations: createSubcountSeries(
					"affiliations",
					affiliationsItems,
					false,
					globalMetrics,
				...
				publishers: createSubcountSeries(
					"publishers",
					publishersItems,
					false,
					globalMetrics,
				...
				periodicals: createSubcountSeries(
					"periodicals",
					periodicalsItems,
					false,
					globalMetrics,
				...
				fileTypes: createSubcountSeries(
					"fileTypes",
					fileTypesItems,
					false,
					globalMetrics,
		...
		usageDeltaData: (() => {
			...
			return {
				global: globalMetrics,
				accessStatuses: createUsageSubcountSeries(
					"accessStatuses",
					accessStatusesItems,
					false,
					globalMetrics,
				...
				fileTypes: createUsageSubcountSeries(
					"fileTypes",
					fileTypesItems,
					false,
					globalMetrics,
				...
				languages: createUsageSubcountSeries(
					"languages",
					languagesItems,
					false,
					globalMetrics,
				...
				resourceTypes: createUsageSubcountSeries(
					"resourceTypes",
					resourceTypesItems,
					false,
					globalMetrics,
				...
				publishers: createUsageSubcountSeries(
					"publishers",
					publishersItems,
					false,
					globalMetrics,
				...
				rights: createUsageSubcountSeries(
					"rights",
					rightsItems,
					false,
					globalMetrics,
				...
				countries: createUsageSubcountSeries(
					"countries",
					countriesItems,
					false,
					globalMetrics,
				...
				affiliations: createUsageSubcountSeries(
					"affiliations",
					affiliationsItems,
					false,
					globalMetrics,
		...
		usageSnapshotData: (() => {
			...
			return {
				...
				countriesByView: createUsageSubcountSeries(
					"countriesByView",
					countriesItems,
					true,
					globalMetrics,
				...
				countriesByDownload: createUsageSubcountSeries(
					"countriesByDownload",
					countriesItems,
					true,
					globalMetrics,
				...
				publishersByView: createUsageSubcountSeries(
					"publishersByView",
					publishersItems,
					true,
					globalMetrics,
				...
				publishersByDownload: createUsageSubcountSeries(
				...
				rightsByView: createUsageSubcountSeries(
					"rightsByView",
					rightsItems,
					true,
					globalMetrics,
				...
				rightsByDownload: createUsageSubcountSeries(
					"rightsByDownload",
					rightsItems,
					true,
					globalMetrics,
 ...
 */
async function generateTestStatsData(startDate, endDate) {
