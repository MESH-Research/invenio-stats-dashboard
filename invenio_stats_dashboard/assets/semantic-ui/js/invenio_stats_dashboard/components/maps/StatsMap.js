...
const StatsMap = ({
	...
	const option = {
		...
		tooltip: {
			...
			formatter: (params) => {
				const countryName =
					params.data?.readableName || params.data?.originalName || params.name;
			...
			? {
					...
					pieces: [
						{ min: 0, max: 0, color: "#b3cde3" },
						{
							min: 0.0001,
							max: Infinity,
							color: lightenAndDesaturate(
								CHART_COLORS.secondary[1][1],
								0.3,
								0.05,
							),
						}, // Light, desaturated orange
					],
				}
			: {
					type: "continuous",
	...
	return (
		<>
			<Header as="h3" attached="top" className="rel-mt-1 stats-map">
				{title}
				<Popup
					trigger={
						<Icon
					...
					content={
						"We are not yet able to provide reliable data on the country of " +
						"origin for visits. This represents the counties from which we " +
						"have confirmed visits, but visitors from other regions may not have " +
						"been counted. We are working to provide better geolocation data in future."
					}
					hoverable
					offset={[-14, 0]}
				/>
