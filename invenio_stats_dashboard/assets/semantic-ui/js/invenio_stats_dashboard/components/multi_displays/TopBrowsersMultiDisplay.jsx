...
const TopBrowsersMultiDisplay = ({
  title = undefined,
  icon = "browser",
  pageSize = 10,
  headers = [i18next.t("Browser"), i18next.t("Visits")],
  default_view,
  ...
  const transformedData = stats.browsers?.slice(0, pageSize).map((browser, index) => ({
    name: browser.name,
    value: browser.count,
  ...
  const remainingItems = stats.browsers?.slice(pageSize) || [];
  const otherData = remainingItems.length > 0 ? remainingItems.reduce((acc, browser) => {
    acc.value += browser.count;
    acc.percentage += browser.percentage;
    return acc;
  }, {
    id: "other",
    name: "Other",
...
TopBrowsersMultiDisplay.propTypes = {
...
export { TopBrowsersMultiDisplay };