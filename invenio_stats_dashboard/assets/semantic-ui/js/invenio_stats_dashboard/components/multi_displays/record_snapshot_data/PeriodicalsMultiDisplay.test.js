...
import { PeriodicalsMultiDisplay } from './PeriodicalsMultiDisplay';
...
describe('PeriodicalsMultiDisplay', () => {
  ...
  describe('transformMultiDisplayData (helper function)', () => {
    ...
    it('should transform valid periodicals data correctly', () => {
      const mockData = [
        {
          id: 'periodical-1',
          name: 'Journal One',
    ...
    it('should handle data with missing or invalid values', () => {
      ...
      expect(result.transformedData[0].value).toBe(100); // Journal One
      expect(result.transformedData[1].value).toBe(50); // Journal Three
    });
    ...
    it('should create correct links for each periodical', () => {
      const mockData = [
        {
          id: 'periodical-1',
          name: 'Journal One',
  ...
  describe('assembleMultiDisplayRows (helper function)', () => {
    it('should assemble rows correctly with transformed data only', () => {
      const transformedData = [
        { id: 'periodical-1', name: 'Journal One', value: 100, percentage: 67, link: '/search?q=custom_fields.journal\\:journal.title:periodical-1' },
        { id: 'periodical-2', name: 'Journal Two', value: 50, percentage: 33, link: '/search?q=custom_fields.journal\\:journal.title:periodical-2' }
      ];
      ...
      expect(rows[0][1].props.href).toBe('/search?q=custom_fields.journal\\:journal.title:periodical-1');
      expect(rows[0][1].props.children).toBe('Journal One');
    ...
    it('should handle data without links', () => {
      const transformedData = [
        { id: 'periodical-1', name: 'Journal One', value: 100, percentage: 67 }
      ];
      ...
      expect(rows[0][0]).toBe(null);
      expect(rows[0][1]).toBe('Journal One');
      expect(rows[0][2]).toBe('100 (67%)');
    });
  ...
  describe('filterSeriesArrayByDate integration', () => {
    it('should filter periodicals data by date range correctly', () => {
      const mockData = [
        {
          id: 'periodical-1',
          name: 'Journal One',
  ...
  describe('Component rendering with different display types', () => {
    it('should render with list configuration', () => {
      ...
      expect(screen.getByText('Journal Two')).toBeInTheDocument();
    ...
    it('should render with custom available_views', () => {
      ...
      expect(screen.getByText('Journal One')).toBeInTheDocument();
      expect(screen.getByText('Journal Two')).toBeInTheDocument();
      expect(screen.getByText('Journal Three')).toBeInTheDocument();
    });
    ...
    it('should render with custom title and icon', () => {
      render(<PeriodicalsMultiDisplay title="Custom Title" icon="newspaper" default_view="list" />);
    ...
    it('should render proper table structure with headers', () => {
      ...
      expect(headerCells).toHaveLength(3); // Icon column, Periodical, Works

      // The header cells should contain the translated text
      expect(headerCells[1]).toHaveTextContent('Periodical');
    ...
    it('should render table rows with proper structure', () => {
      ...
      expect(dataRows).toHaveLength(3); // 3 periodicals
      ...
      // Check that the first data cell contains the periodical name
      expect(firstRowCells[1]).toHaveTextContent('Journal One');
